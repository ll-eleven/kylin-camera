/****************************************************************************
** Meta object code from reading C++ file 'camerapage.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.12.8)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../src/camerapage.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'camerapage.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.12.8. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_CameraPage_t {
    QByteArrayData data[12];
    char stringdata0[169];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CameraPage_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CameraPage_t qt_meta_stringdata_CameraPage = {
    {
QT_MOC_LITERAL(0, 0, 10), // "CameraPage"
QT_MOC_LITERAL(1, 11, 16), // "updateRecordTime"
QT_MOC_LITERAL(2, 28, 0), // ""
QT_MOC_LITERAL(3, 29, 18), // "displayCameraError"
QT_MOC_LITERAL(4, 48, 9), // "timeEvent"
QT_MOC_LITERAL(5, 58, 15), // "creatCameraPage"
QT_MOC_LITERAL(6, 74, 11), // "const char*"
QT_MOC_LITERAL(7, 86, 16), // "camera_set_param"
QT_MOC_LITERAL(8, 103, 16), // "KylinCameraInfo*"
QT_MOC_LITERAL(9, 120, 13), // "change_device"
QT_MOC_LITERAL(10, 134, 17), // "change_resolution"
QT_MOC_LITERAL(11, 152, 16) // "QPair<uint,uint>"

    },
    "CameraPage\0updateRecordTime\0\0"
    "displayCameraError\0timeEvent\0"
    "creatCameraPage\0const char*\0"
    "camera_set_param\0KylinCameraInfo*\0"
    "change_device\0change_resolution\0"
    "QPair<uint,uint>"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CameraPage[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   49,    2, 0x0a /* Public */,
       3,    0,   50,    2, 0x0a /* Public */,
       4,    0,   51,    2, 0x0a /* Public */,
       5,    1,   52,    2, 0x0a /* Public */,
       7,    1,   55,    2, 0x0a /* Public */,
       9,    1,   58,    2, 0x0a /* Public */,
      10,    1,   61,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, 0x80000000 | 6,    2,
    QMetaType::Void, 0x80000000 | 8,    2,
    QMetaType::Void, 0x80000000 | 6,    2,
    QMetaType::Void, 0x80000000 | 11,    2,

       0        // eod
};

void CameraPage::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<CameraPage *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->updateRecordTime(); break;
        case 1: _t->displayCameraError(); break;
        case 2: _t->timeEvent(); break;
        case 3: _t->creatCameraPage((*reinterpret_cast< const char*(*)>(_a[1]))); break;
        case 4: _t->camera_set_param((*reinterpret_cast< KylinCameraInfo*(*)>(_a[1]))); break;
        case 5: _t->change_device((*reinterpret_cast< const char*(*)>(_a[1]))); break;
        case 6: _t->change_resolution((*reinterpret_cast< QPair<uint,uint>(*)>(_a[1]))); break;
        default: ;
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject CameraPage::staticMetaObject = { {
    &QStackedWidget::staticMetaObject,
    qt_meta_stringdata_CameraPage.data,
    qt_meta_data_CameraPage,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *CameraPage::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CameraPage::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_CameraPage.stringdata0))
        return static_cast<void*>(this);
    return QStackedWidget::qt_metacast(_clname);
}

int CameraPage::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QStackedWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 7)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 7;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
